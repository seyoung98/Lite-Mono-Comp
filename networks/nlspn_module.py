...
class NLSPN(nn.Module):
    def __init__(self, options, ch_g, ch_f, k_g, k_f):
        ...
        self.opt = options
        self.prop_time = self.opt.prop_time
        self.affinity = self.opt.affinity
        ...
        if self.affinity in ['AS', 'ASS', 'TC', 'TGASS']:
            ...
            if self.affinity == 'TC':
                ...
                self.aff_scale_const = nn.Parameter(
                    self.opt.affinity_gamma * self.num * torch.ones(1))
            else:
