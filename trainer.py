...
class Trainer:
    def __init__(self, options):
        ...
        """ Opimization """
        # 1. Load The Optimization Algo (ex. Adam, AdamW)
        ...
        """ Load DataSet"""
        # data
        data = get_data(self.opt)

        data_train = data(self.opt, 'train')
        data_val = data(self.opt, 'val')
        img_ext = '.png' if self.opt.png else '.jpg'

        # 1. Define dataset for Pytorch DataLoader

        self.train_loader = DataLoader(
            dataset=data_train, batch_size=self.opt.batch_size, shuffle=False,
            num_workers=self.opt.num_threads, pin_memory=True,
            drop_last=True)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            # Compute Loss
            outputs, losses = self.embedding(inputs)

            # Start Optimization with Loss
    ...
    def embedding(self, inputs, mode='rgbd'):

        # Encoder
        global conv1_rgb, conv1_dep, fe1
        if mode == 'rgbd':
            conv1_rgb = self.conv_bn_relu(3, 48, kernel=3, stride=1, padding=1,
                                          bn=False)
            conv1_dep = self.conv_bn_relu(1, 16, kernel=3, stride=1, padding=1,
                                          bn=False)
            conv1 = self.conv_bn_relu(64, 64, kernel=3, stride=1, padding=1,
                                      bn=False)
        elif mode == 'rgb':
            conv1 = self.conv_bn_relu(3, 64, kernel=3, stride=1, padding=1,
                                      bn=False)
        elif mode == 'd':
            conv1 = self.conv_bn_relu(1, 64, kernel=3, stride=1, padding=1,
                                      bn=False)
        else:
        ...
        if mode == 'rgbd':
            fe1_rgb = conv1_rgb(rgb)
            fe1_dep = conv1_dep(dep)
        ...
        # features = {}
        features = self.models["encoder"](fe1)
...
#         return outputs, losses
    ...
    def load_pretrain(self):
        ...
        try:
            self.models["encoder"].load_state_dict(model_dict, strict=False)
    ...
    def load_model(self):
        ...
        for n in self.opt.models_to_load:
            ...
            if n in ['pose_encoder', 'pose']:
                ...
                model_dict.update(pretrained_dict)
                self.models_pose[n].load_state_dict(model_dict, strict=False)
            else:
        ...
        if os.path.isfile(optimizer_load_path):
            ...
            self.model_optimizer.load_state_dict(optimizer_dict, strict=False)
            self.model_pose_optimizer.load_state_dict(optimizer_pose_dict, strict=False)
        else:
